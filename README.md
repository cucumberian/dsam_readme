...
### Путь до бота
Бот хранится в каталоге `/root/script/google-sheet-telegram-bot`
...
### Резюме
Для подключения к уже рабочему сеансу tmux удобно использовать команду
```bash
tmux a
```
Для отсоединения от сеанса без его закрытия есть комбинация клавиш `Ctrl+b d`
...
## Midnight Commander
Для визуальной навигации можно использовать панельный менеджер **Midnight Commander**.
...
## Резюме
**Подключение** через консоль командой
```bash 
ssh имя_пользователя@адрес_сервера
```
**Узнать статус** бота
```bash
systemctl status dsam-telbot.service
```

**Просмотр логов** команда
```bash
journalctl -xefu dsam-telbot.service
```
**Запуск бота**
