...
## Логи бота
Логи храняться в каталоге `/root/script/google-sheet-telegram-bot/logs`
...
- Просмотр всех текущих открытых сессий производится командой
```bash
tmux ls
...
- Подключение к последней открытой сессии:
```bash
tmux a
```
...
**Просмотр** файла под курсором - F3
Список действий через функциональные клавиши - внизу экрана.